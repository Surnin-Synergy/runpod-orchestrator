...
export class Coordinator extends EventEmitter {
  ...
  async processJob(clientJobId: string): Promise<void> {
    try {
      // Try to acquire lock
      const lockResult = await this.redisUtils.acquireLock(
        clientJobId,
        this.instanceId
      );
      ...
      try {
        ...
        this.log(
          "error",
          `Error releasing lock for ${clientJobId}:`,
          releaseError
        );
      }
    }
  ...
  private async handleSubmittedJob(
    ...
    try {
      ...
      if (success) {
        ...
        this.log(
          "info",
          `Job ${job.clientJobId} submitted to Runpod: ${runpodResponse.id} (endpoint: ${job.endpointId})`
        );
      }
    } catch (error) {
  ...
  private async handleActiveJob(
    ...
    try {
      ...
      const mappedStatus = this.runpodClient.mapRunpodStatus(
        runpodStatus.status
      );

      // Update job with runpodStatus for persistence
      ...
      if (TERMINAL_STATUSES.includes(mappedStatus as RunpodTaskStatus)) {
        await this.handleTerminalStatus(
          job,
          mappedStatus as RunpodTaskStatus,
          runpodStatus,
          lockToken
        );
      ...
      this.log(
        "error",
        `Error checking status for job ${job.clientJobId}:`,
        error
      );
  ...
  private async handleTerminalStatus(
    ...
    if (success) {
      ...
      this.log(
        "info",
        `Job ${job.clientJobId} reached terminal state: ${status}`
      );
    }
